...
\author[ ]{\normalsize{
        ...
        Tishya Dhakal(Roll no. 7)\linebreak
...
The exponential growth of scientific literature has rendered traditional bibliometric indicators citation counts, H-indices inadequate for early-stage discovery of breakthrough research. These retrospective metrics consistently lag behind genuine intellectual shifts, leaving funding agencies and researchers unable to identify high-potential work before it reaches saturation.
This project proposes EmerG-Net, a temporal heterogeneous graph modeling framework designed to detect and rank emerging research communities before they achieve mainstream recognition. The central contribution is a synthetic predictive metric \textbf{the Boom Score} which quantifies a community's future impact across three dimensions: Awakening Time (deviation from linear citation growth), Structural Growth (density of time-respecting citation paths), and Topical Shift (rate of new keyword emergence).

The framework integrates SciBERT-based semantic embeddings with graph-structural representations learned via node2vec and Graph Neural Networks, unified through a Deep Residual Graph Neural Network (DrGNN) architecture to address the oversmoothing problem in deep graph models. Community detection is performed using Leiden and Infomap algorithms on temporal graph snapshots, enabling tracking of community lifecycle events across time.
Applied to scholarly citation networks modeled as Scholarly Knowledge Graphs (SKGs), EmerG-Net produces ranked community predictions interpretable as early investment signals for researchers and institutions. The framework is designed for modularity and extensibility beyond academic citation data.

\textit{Keywords: Scholarly Knowledge Graphs, Temporal Graph Neural Networks, Community Detection, Bibliometrics, Boom Score, SciBERT, DrGNN, Emerging Research Detection, Sleeping Beauty, Citation Analysis}
\newpage
...
\subsection{Ranking of Communities}
For the ranking of communities, we will compute the \textbf{"Boom Score"} for each community 
using Dense Layers of appropriate dimensions. The model will be trained using a supervised 
learning approach, where we will label communities based on their historical citation 
trajectories (e.g., "Booming" vs. "Non-Booming") and optimize the model to predict these 
labels accurately.
